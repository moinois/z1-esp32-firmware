...
def test_mock_sd_unmount_reports_absence_and_remount_recovers(
    ...
    try:
        ...
        listed = _payload(tcp.exchange(GENERAL_COMMAND, b"ls /sd", 5.0))
        assert listed == b"Load directory finished.\r\n"
        with pytest.raises(FileTransferError, match="failed to open file"):
            upload_file(usb_client, "/sd/ABSENT.BIN", b"must-not-write")

        assert _control(tcp, "mount") == b"mock-sd mounted\n"
        upload_file(usb_client, "/sd/RECOVER.BIN", b"mounted-again")
        assert download_file(usb_client, "/sd/RECOVER.BIN") == b"mounted-again"
    finally:
        _control(usb_client, "clear")
        if _control(usb_client, "status") == b"mock-sd unmounted\n":
            _control(usb_client, "mount")
        try:
            usb_client.exchange(GENERAL_COMMAND, b"rm /sd/RECOVER.BIN", 4.0)
        except Exception:
            pass
...
def test_mock_sd_latched_read_write_and_sync_faults_recover(
    ...
    try:
        ...
        upload_file(usb_client, "/sd/WRITE.BIN", b"write-recovered")
        assert download_file(usb_client, "/sd/WRITE.BIN") == b"write-recovered"

        assert _control(usb_client, "fail-sync") == b"mock-sd fail-sync armed\n"
        failed = _payload(
            usb_client.exchange(GENERAL_COMMAND, b"mkdir /sd/SYNCFAIL", 5.0)
        ).lower()
        assert b"created directory" not in failed
        _control(usb_client, "clear")
        recovered = _payload(
            usb_client.exchange(GENERAL_COMMAND, b"mkdir /sd/SYNCOK", 5.0)
        ).lower()
        ...
        if _control(usb_client, "status") == b"mock-sd unmounted\n":
            _control(usb_client, "mount")
        for path in ("/sd/READ.BIN", "/sd/WRITE.BIN", "/sd/SYNCFAIL", "/sd/SYNCOK"):
            try:
                usb_client.exchange(
