...
from tools.controller_payload_catalog import encoded_version, load_catalog, resolve_payload, workflow_options
...
def test_catalog_and_workflow_options_are_synchronized() -> None:
    ...
    assert workflow_options(workflow) == [
        "None (do not update LPC)",
        *(str(payload["display_name"]) for payload in catalog.values()),
    ]
...
def test_official_payload_identity_and_encoded_version() -> None:
    ...
    payload_id, resolved = resolve_payload(load_catalog(Path("release/controller-payloads.json")), str(payload["display_name"]))
    assert payload_id == "official-z1-v1.1.2"
