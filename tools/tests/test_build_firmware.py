...
class BuildFirmwareTests(unittest.TestCase):
    ...
    def test_release_enables_size_optimization_and_rejects_mocks(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("CONFIG_COMPILER_OPTIMIZATION_ASSERTIONS_SILENT=y", release_text)
            self.assertIn("CONFIG_COMPILER_OPTIMIZATION_CHECKS_SILENT=y", release_text)
            self.assertIn("CONFIG_BT_STACK_NO_LOG=y", release_text)
            self.assertIn("CONFIG_BOOTLOADER_LOG_LEVEL_WARN=y", release_text)
