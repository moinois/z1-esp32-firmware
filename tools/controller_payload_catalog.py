...
def workflow_options(text: str) -> list[str]:
    ...
    return re.findall(r"^\s+- ([a-z0-9][a-z0-9.-]*)\s*$", generated, re.MULTILINE)
...
def main() -> int:
    ...
    if args.payload_id not in payloads:
        raise SystemExit(f"unknown controller payload: {args.payload_id}")
    payload = payloads[args.payload_id]
    values = {**payload, "payload_id": args.payload_id,
