...
| CAN-002--CAN-003, CAN-005--CAN-006 CANopen node identity and NMT | Portable core node state machine | Yes for derived identifiers, boot-up, retained heartbeat timing, addressed/broadcast NMT, delayed reset, and observed error bits | Dictionary composition, 10 ms scheduler, restart, and TWAI adapters pending |
...
| CAN-002 SDO configuration | Portable expedited SDO server and named timing policy | Yes for server/client timeout values, block disablement, exact request identifiers, scalar uploads/downloads, and aborts | 10 ms service composition and TWAI adapter pending |
