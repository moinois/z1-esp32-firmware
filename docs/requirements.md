...
| CAN-002--CAN-003, CAN-005--CAN-006 CANopen node identity and NMT | Portable core node state machine and application composition service | Yes for derived identifiers, boot-up, retained heartbeat timing, addressed/broadcast NMT, delayed reset, observed error bits, dictionary consistency, and external effects | 10 ms target scheduler, restart, and TWAI service-loop adapters pending |
...
| CAN-002 SDO configuration | Portable expedited SDO server, named timing policy, and composed write effects | Yes for server/client timeout values, block disablement, exact request identifiers, scalar uploads/downloads, aborts, heartbeat updates, and digital diagnostics | 10 ms target scheduler and TWAI service-loop adapter pending |
...
| CAN-001, HW-050--HW-052 TWAI hardware | Portable configuration plus ESP-IDF classic-CAN adapter | Exact mode, rate, timing, pins, filter, queues, unused outputs, installation, and frame conversion host/target-build verified | CAN service-loop startup and physical bus verification pending |
| DIAG-033 CAN digital-output monitoring | Application sampler with replaceable informational-log port | Yes for exact startup records, nominal period constant, initial zero, changed samples, collapse, hex format, and DO2 bit | CAN target-task scheduling and ESP-IDF log adapter pending |
