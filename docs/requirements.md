...
| BLE-010--BLE-016 provisioning commands | Application implementation with replaceable Wi-Fi/report/diagnostic port | Yes for security gates, credentials, connect/disconnect, reports, scan, errors, and custom data | Runtime BLUFI composition and ESP-IDF Wi-Fi adapter pending |
...
| BWF-010--BWF-023, BWF-033 frame envelope, security, and acknowledgement | Application implementation with replaceable cipher and notification port | Yes for exact envelope, sequences, security flags, checksum order, encryption scope, failures, and acknowledgement timing | Product dispatch and ESP-IDF GATT adapter pending |
